...
  try {
    // CRITICAL FIX: Gracefully handle empty body to prevent crash (socket hang up)
    if (!req.body || Object.keys(req.body).length === 0) {
        logger.warn('Login request failed: Empty body received');
        return res.status(400).json({ success: false, message: 'Request body must contain email and password.' });
    }
